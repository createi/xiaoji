<template>
  <div class="agent-apply-page">
    <PageHeader title="分销申请" />
    <SearchForm>
      <a-form layout="inline" @finish="handleSearch">
        <a-form-item label="申请人">
          <a-input v-model:value="searchParams.userId" placeholder="请输入申请人ID" allow-clear />
        </a-form-item>
...
import { getAgentApplyList, approveAgentApply, rejectAgentApply } from '@/api/agent'
...
async function handleApprove(record: any) {
  try {
    await approveAgentApply(record.id)
    message.success('已通过')
    fetchData()
  } catch {
    // silent
...
async function handleReject(record: any) {
  try {
    await rejectAgentApply(record.id)
    message.success('已拒绝')
    fetchData()
  } catch {
    // silent
